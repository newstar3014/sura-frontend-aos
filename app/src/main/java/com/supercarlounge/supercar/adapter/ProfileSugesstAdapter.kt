...
) : RecyclerView.Adapter<ProfileSugesstAdapter.ViewHolder>() {
    var selectindex = -1
...
//    var mDeviceH = deviceH
    ...
    inner class ViewHolder(parent: ViewGroup) : RecyclerView.ViewHolder(
        LayoutInflater.from(parent.context).inflate(R.layout.item_profile_suggestion, parent, false)
    ) {
        private val binding: ItemProfileSuggestionBinding? = DataBindingUtil.bind(itemView)
...
//                        var bitmap: Bitmap = image_task.execute().get()
                            ...
                            if (type == 0){
                                if (binding?.checkset == true){
                                    (con as MainActivity).runOnUiThread {
...
//                                    .into(binding.ivProfile)
                                ...
                                (con as PrvMainActivity).runOnUiThread {
                                    Glide.with(con).load(data.u_image)
...
//                    .capture(data.u_image).get()
    ...
    companion object {
        ...
        @BindingAdapter("app:profilesugesst","app:posi","app:width")
        @JvmStatic
        fun setItems(recyclerView: RecyclerView, items: ArrayList<ProfileSuggestionData>,posi:Int,width:Int) {
            var adapter = (recyclerView.adapter as ProfileSugesstAdapter?)
            items?.let {
                adapter?.datalist = it
                if (it.size != 0) {
...
//                if (posi!= 0){
//                    pos = posi
////                    val offset = (recyclerView.width / 2 - width / 1.5)
////                    var mnan = (recyclerView.layoutManager!! as LinearLayoutManager)
////                    mnan.scrollToPositionWithOffset(pos, offset.toInt() )
//                }else{
...
//                this.width = if (isTranslateSize) displayPW.div(5.8).toInt() else displayPW.div(5.8).toInt()
                ...
                // 3개씩 보여주게 수정
                this.height = if (isTranslateSize) displayPW.div(3).toInt() else displayPW.div(3).toInt()
                this.width = if (isTranslateSize) displayPW.div(3).toInt() else displayPW.div(3).toInt()
...
//                view.scaleY =if (isTranslateSize) 1.05F else 0.9F
                ...
                // 3개씩 보여주게 수정
                view.scaleX =if (isTranslateSize) 1.15F else 0.9F
                view.scaleY =if (isTranslateSize) 1.15F else 0.9F
            }
        }
