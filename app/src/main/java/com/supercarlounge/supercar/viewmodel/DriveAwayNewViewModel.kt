...
class DriveAwayNewViewModel : ViewModel() {
    ...
    fun getDrive(u_seq:String,isreset:Boolean,binding:   FragmentDriveAwayNewBinding){
        issend.value = true
        Log.d("스크롤홈확인용3 : last","로딩 시작" )

        var list :ArrayList<DriveAwayData> = arrayListOf()
        CoroutineScope(Dispatchers.IO).launch {
            val d = NetworkManager.serveradapter!!.create(DriveService::class.java).getDrive("신규",u_seq,ppp.value,page.value,"0","0"
            )
