...
class SwipeHelperCallback2(private val recyclerViewAdapter : ProfileMatchingAdapter, val event:(Int, Boolean)->Unit)  : ItemTouchHelper.Callback() {
    ...
    override fun onSelectedChanged(viewHolder: RecyclerView.ViewHolder?, actionState: Int) {
        viewHolder?.let {
            ...
            event(currentPosition!!,false)
//            viewModel.swipe.value = false
//            viewModel.cursorPosition.value =currentPosition
            Log.d("스와이프", "onSelectedChanged: ")
        }
    }
    ...
    override fun onChildDraw(
        ...
        if (actionState == ACTION_STATE_SWIPE) {
            ...
            var newX = clampViewPositionHorizontal(dX, isClamped, isCurrentlyActive)  // newX 만큼 이동(고정 시 이동 위치/고정 해제 시 이동 위치 결정)
            newX = (newX /1.2).toFloat()
            // 고정시킬 시 애니메이션 추가
            if (newX == -clamp) {
    ...
    private fun setTag(viewHolder: RecyclerView.ViewHolder, isClamped: Boolean) {
        ...
        Log.d("스와이프", isClamped.toString())
        var position =viewHolder.adapterPosition
        event(position,isClamped)
       // viewModel.swipe.value = isClamped
    }
